...
class MyLib_OpenPNE_KtaiEmoji_Common
{
    var $carrier_id;
    var $value_list;
     ...
     * constructor
     */
    function MyLib_OpenPNE_KtaiEmoji_Common()
    {
        $this->carrier_id = '';
    ...
    static function &getInstance()
    {
        static $singleton;
        if (empty($singleton)) {
            $singleton = new MyLib_OpenPNE_KtaiEmoji_Common();
        }
        return $singleton;
    }
     ...
     * 絵文字が存在しない場合はfalseを返す
     */
    function get_emoji_utf4unicode($emoji)
    {
        return array_search($emoji, $this->utf2unicode);
    }
    ...
    function get_unicode4emoji_code_id($emoji_code_id)
    {
        return $this->unicode_value_list[$emoji_code_id];
    }
}
